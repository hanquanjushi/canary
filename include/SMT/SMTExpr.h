...
#define DECLARE_ARRAY_OPERATION(X) \
	 ...
	 * Existential quantifier elimination facilities.
	 ...
	 "qe2" is based on "Playing with quantified satisfaction, LPAR 15" that uses "model based projection" proposed in CAV 14.

	 We use the name "forget" because this procedure(or its variant) is often called the "forget operator" both by the abstract interpretation community and the artificial intelligence community.
	...
	std::pair<bool, SMTExpr> forgetVars(SMTExprVec &Vars, unsigned Timeout =
			2000);

	/********************
	 *  Generalized consequence finding or symbolic abstraction
	 *
	 *  Given P(x,y,z), find its strongest consequence/best abstraction in an abstract domain.
	 *
	 *  TODO:
	...
	void abstractIntervalAsExprvec(SMTExpr &Var, SMTExprVec &Evec, int Timeout =
			5000);
