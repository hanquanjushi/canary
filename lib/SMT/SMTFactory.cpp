...
SMTExprVec SMTFactory::translate(const SMTExprVec &Exprs) {
	...
	std::shared_ptr<z3::expr_vector> Vec(
			new z3::expr_vector(
					z3::expr_vector(Ctx,
							Z3_ast_vector_translate(Exprs.ExprVec->ctx(),
									*Exprs.ExprVec, Ctx))));
...
SMTExpr SMTFactory::translate(const SMTExpr &Expr) {
	...
	z3::expr RetExpr = z3::expr(Ctx,
			Z3_translate(Expr.Expr.ctx(), Expr.Expr, Ctx));
	SMTExpr Ret(this, RetExpr);
	return Ret;
}

std::pair<SMTExprVec, bool> SMTFactory::rename(const SMTExprVec &Exprs,
	...
	for (unsigned ExprIdx = 0; ExprIdx < Exprs.size(); ExprIdx++) {
		...
		if (It != ExprRenamingCache.end()) {
			...
			bool AllPruned = visit(Ret, LocalMapping, ToPrune, Visited,
					Advisor);
			if (AllPruned) {
				RenamingUtility RU { true, createBoolVal(true),
						createEmptySMTExprVec(), LocalMapping };
				ExprRenamingCache.insert(std::make_pair(Ret, RU));

				RetBool = true;
				continue;
			} else if (ToPrune.size()) {
				SMTExprVec TrueVec = this->createBoolSMTExprVec(true,
						ToPrune.size());
				assert(ToPrune.size() == TrueVec.size());
				SMTExpr AfterSubstitution = Ret.substitute(ToPrune, TrueVec);
				RenamingUtility RU { true, AfterSubstitution, ToPrune,
						LocalMapping };
				ExprRenamingCache.insert(std::make_pair(Ret, RU));
				...
				RenamingUtility RU { false, createBoolVal(true),
						createEmptySMTExprVec(), LocalMapping };
				ExprRenamingCache.insert(std::make_pair(Ret, RU));
			}
		}
		...
		if (RenamingSuffix != "") {
			...
			SMTExprVec From = createEmptySMTExprVec(), To =
					createEmptySMTExprVec();
			...
			for (auto &It : LocalMapping) {
				...
				if (!Advisor || Advisor->rename(OldExpr)) {
					...
					SMTExpr NewExpr(this,
							z3::expr(Ctx,
									Z3_mk_const(Ctx,
											Z3_mk_string_symbol(Ctx,
													NewSymbol.c_str()),
											OldExpr.Expr.get_sort())));
					Mapping.insert(
							std::pair<std::string, SMTExpr>(OldSymbol,
									NewExpr));
...
bool SMTFactory::visit(SMTExpr &Expr2Visit,
	...
	if (Visited.count(Expr2Visit)) {
		...
		if (It != ExprRenamingCache.end()) {
			...
			Mapping.insert(Cache.SymbolMapping.begin(),
					Cache.SymbolMapping.end());
		...
		if (Expr2Visit.isConst() && !Expr2Visit.isNumeral()) {
			if (Advisor && Advisor->prune(Expr2Visit)) {
				...
				if (!Expr2Visit.isTrue() && !Expr2Visit.isFalse()) {
					...
					Mapping.insert(
							std::pair<std::string, SMTExpr>(Symbol,
									Expr2Visit));
				}
			}
...
SMTExprVec SMTFactory::createSMTExprVec(const std::vector<SMTExpr> &ExprVec) {
	...
	for (size_t J = 0; J < ExprVec.size(); J++) {
		...
		assert(
				&Ret.getSMTFactory() == &Ex.getSMTFactory()
...
SMTExpr SMTFactory::createStore(SMTExpr &Vec, SMTExpr Index,
	...
	return SMTExpr(this,
			z3::expr(Ctx,
					Z3_mk_store(Ctx, Vec.Expr, Index.Expr, Val2Store.Expr)));
}

SMTExpr SMTFactory::createIntRealArrayConstFromStringSymbol(
		const std::string &Name) {
	Z3_sort ArraySort = Ctx.array_sort(Ctx.int_sort(), Ctx.real_sort());
	return SMTExpr(this,
			z3::expr(Ctx,
					Z3_mk_const(Ctx, Z3_mk_string_symbol(Ctx, Name.c_str()),
							ArraySort)));
}

SMTExpr SMTFactory::createIntBvArrayConstFromStringSymbol(
		const std::string &Name, uint64_t Sz) {
...
SMTExpr SMTFactory::createIntDomainConstantArray(SMTExpr &ElmtExpr) {
	return SMTExpr(this,
			z3::expr(Ctx,
					Z3_mk_const_array(Ctx, Ctx.int_sort(), ElmtExpr.Expr)));
}
