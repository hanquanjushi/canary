...
SMTExpr::SMTExpr(SMTFactory *F, z3::expr Z3Expr) :
		SMTObject(F), Expr(Z3Expr) {
}

SMTExpr::SMTExpr(SMTExpr const &E) :
		SMTObject(E), Expr(E.Expr) {
}
...
SMTExpr SMTExpr::substitute(SMTExprVec &From, SMTExprVec &To) {
	...
	return SMTExpr(&getSMTFactory(),
			Expr.substitute(*From.ExprVec, *To.ExprVec));
}
...
std::pair<int, int> SMTExpr::abstractInterval(SMTExpr &E, int Timeout) {
	...
	z3::set_param("smt.timeout", Timeout); //p.set("timeout", Timeout); TODO: it seems we cannot set timeout directly to opt
...
void SMTExpr::abstractIntervalAsExprvec(SMTExpr &E, SMTExprVec &Evec,
	...
	try {
		if (LowerFinder.check() == z3::sat) {
			...
			Evec.push_back(
					SMTExpr(&getSMTFactory(), LowerFinder.upper(LowerGoal)));
		}
	} catch (z3::exception &Ex) {
		Evec.push_back(SMTExpr(&getSMTFactory(), Ctx.bool_val(false)));
	}
	...
	try {
		if (UpperFinder.check() == z3::sat) {
			...
			Evec.push_back(
					SMTExpr(&getSMTFactory(), UpperFinder.lower(UpperGoal)));
		}
	} catch (z3::exception &Ex) {
		// timeout
		Evec.push_back(SMTExpr(&getSMTFactory(), Ctx.bool_val(false)));
...
SMTExpr SMTExpr::dilligSimplify(SMTExpr N, z3::solver &Solver4Sim,
	...
	if (!N.isLogicAnd() && !N.isLogicOr()) {
		...
		if (N.isLogicAnd()) {
			...
			SMTExpr Ret(&getSMTFactory(),
					to_expr(Ctx, Z3_mk_and(Ctx, J, Args)));
			delete[] Args;
...
// Quantifier elimination, some contextual simplifications such as dillig-Simpify, function minimization, etc.
...
SMTExpr SMTExpr::bv12bool() {
	if (Expr.is_array()) {
		...
		assert(bvSz == 1);
		auto func =
				(Expr.ctx().bv_val("0", 1) == Expr.ctx().bv_val("0", 1)).decl();

		z3::expr const_bv1 = const_array(Expr.get_sort().array_domain(),
				Expr.ctx().bv_val("1", 1));
...
SMTExpr SMTExpr::bool2bv1() {
	...
	if (Expr.is_array()) {
		...
		z3::expr const_bv0 = const_array(Expr.get_sort().array_domain(),
				ctx.bv_val(0, 1));
		z3::expr const_bv1 = const_array(Expr.get_sort().array_domain(),
				ctx.bv_val(1, 1));
...
SMTExpr SMTExpr::real2int() {
	...
	if (Expr.is_array()) {
		...
		auto func =
				z3::expr(ctx, Z3_mk_real2int(ctx, ctx.real_val("0.0"))).decl();
		...
		assert(Expr.is_real());
		return SMTExpr(&getSMTFactory(),
				z3::expr(ctx, Z3_mk_real2int(ctx, Expr)));
	}
}
...
SMTExpr SMTExpr::int2bv(unsigned sz) {
	...
	if (Expr.is_array()) {
		assert(Expr.get_sort().array_range().is_int());
		auto func =
				z3::expr(ctx, Z3_mk_int2bv(ctx, sz, ctx.int_val("0"))).decl();
...
SMTExpr SMTExpr::bv2int(bool isSigned) {
	...
	if (Expr.is_array()) {
		...
		auto func = z3::expr(ctx,
				Z3_mk_bv2int(ctx, ctx.bv_val("0", bvSz), isSigned)).decl();
...
SMTExpr SMTExpr::basic_concat(SMTExpr &b) {
	...
	if (Expr.is_array() && b.Expr.is_array()) {
		...
		if (Expr.get_sort().array_range().is_bv()) {
			unsigned bvSz = Expr.get_sort().array_range().bv_size();
			func = z3::expr(ctx,
					Z3_mk_concat(ctx, ctx.bv_val("0", bvSz),
							ctx.bv_val("0", bvSz))).decl();
		} else {
			assert(false);
		}
		return SMTExpr(&getSMTFactory(),
...
SMTExpr SMTExpr::basic_extract(unsigned high, unsigned low) {
	...
	if (Expr.is_array()) {
		...
		unsigned bvSz = Expr.get_sort().array_range().bv_size();
		auto func = z3::expr(ctx,
				Z3_mk_extract(ctx, high, low, ctx.bv_val("10", bvSz))).decl();
		Z3_ast mapargs[1] = { Expr };
		return SMTExpr(&getSMTFactory(),
				z3::expr(ctx, Z3_mk_map(ctx, func, 1, mapargs)));
	} else {
...
SMTExpr SMTExpr::basic_sext(unsigned sz) {
	...
	if (Expr.is_array()) {
		...
		unsigned bvSz = Expr.get_sort().array_range().bv_size();
		auto func = z3::expr(ctx,
				Z3_mk_sign_ext(ctx, sz, ctx.bv_val("10", bvSz))).decl();
		Z3_ast mapargs[1] = { Expr };
		return SMTExpr(&getSMTFactory(),
				z3::expr(ctx, Z3_mk_map(ctx, func, 1, mapargs)));
	} else {
...
SMTExpr SMTExpr::basic_zext(unsigned sz) {
	...
	if (Expr.is_array()) {
		...
		unsigned bvSz = Expr.get_sort().array_range().bv_size();
		auto func = z3::expr(ctx,
				Z3_mk_zero_ext(ctx, sz, ctx.bv_val("10", bvSz))).decl();
		Z3_ast mapargs[1] = { Expr };
		return SMTExpr(&getSMTFactory(),
				z3::expr(ctx, Z3_mk_map(ctx, func, 1, mapargs)));
	} else {
...
SMTExpr SMTExpr::basic_ite(SMTExpr &TBValue, SMTExpr &FBValue) {
	...
	if (condition.is_array()) {
		...
		if (TBValue.Expr.get_sort().array_range().is_bv()) {
			...
			func = ite(ctx.bool_val(true), ctx.bv_val(1, bvSz),
					ctx.bv_val(0, bvSz)).decl();
		} else {
			assert(TBValue.Expr.get_sort().array_range().is_real());
			assert(FBValue.Expr.get_sort().array_range().is_real());
			func = ite(ctx.bool_val(true), ctx.real_val("0.0"),
					ctx.real_val("0.0")).decl();
		}
...
bool SMTExpr::isLogicalEquivTo(SMTExpr const &E) {
	...
	if (!Expr.is_bool() && !E.Expr.is_bool()) {
		...
		// if ((Expr - E.Expr).simplify().to_string() == bvZero.to_string()) return true;
		z3::solver S(Ctx);
		S.add(Expr - E.Expr != 0);
		if (S.check() == z3::unsat) {
			return true;
...
SMTExprVec SMTExprVec::copy() {
	...
	std::shared_ptr<z3::expr_vector> Ret = std::make_shared<z3::expr_vector>(
			ExprVec->ctx());
...
SMTExpr SMTExprVec::toAndExpr() const {
	...
	z3::expr t = ExprVec->ctx().bool_val(true), f = ExprVec->ctx().bool_val(
			false);
