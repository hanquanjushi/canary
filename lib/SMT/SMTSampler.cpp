...
public:
	quick_sampler(std::string input, int max_samples, double max_time) :
			input_file(input), max_samples(max_samples), max_time(max_time), opt(
					c) {
	}
	...
	void sample(z3::model m) {
		...
		for (unsigned i = 0; i < ind.size(); ++i) {
			...
			if (solve()) {
				...
				if (initial_mutations.find(new_string)
						== initial_mutations.end()) {
	...
	z3::expr literal(int v) {
		return c.constant(c.str_symbol(std::to_string(v).c_str()),
				c.bool_sort());
	}
};
