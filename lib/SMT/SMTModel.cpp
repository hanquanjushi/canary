...
SMTModel::SMTModel(SMTModel const &M) : SMTObject(M), Model(M.Model) {}
...
unsigned SMTModel::size() { return Model.size(); }
...
// this is not a good approach, but currently we cannot build a model explicitly
// with C/C++ APIs
